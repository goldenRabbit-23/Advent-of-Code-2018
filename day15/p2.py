...
class Unit:
    def __init__(self, unit_type, r, c, atk):
        self.type = unit_type
        self.r = r
        self.c = c
        self.hp = 200
...
class Game:
    def __init__(self, game_map, elf_attack):
        self.walls = set()
        self.units = []
        ...
        for r, row in enumerate(game_map):
            for c, cell in enumerate(row):
                if cell == '#':
                    ...
                    atk = self.elf_attack if cell == 'E' else 3
                    self.units.append(Unit(cell, r, c, atk))
...
def main():
    ...
    for elf_attack in count(4):
        game = Game(game_map, elf_attack)
        initial_elves = sum(unit.type == 'E' for unit in game.units)
        rounds = 0

        while game.run():
            rounds += 1

        surviving_elves = sum(unit.is_alive and unit.type == 'E' for unit in game.units)
